...
int step_home(void)
{
    ...
    bool homestat = digitalRead(PIN_STP_HOME);
    //while (digitalRead(PIN_STP_HOME) == false) {
    while (homestat == false) {
        if (digitalRead(PIN_STP_FAULT) == false) {
            /* Put the stepper back in sleep mode. */
            digitalWrite(PIN_STP_SLEEP,false);
            Serial.println("stepper overcurrent or overtemperature");
            sprintf(err, "stepper driver detected a fault");
            return -1;
        }
        digitalWrite(PIN_STP_STEP,true);
        ...
        delayMicroseconds(UDELAY);
        Serial.println("hey");
        homestat = digitalRead(PIN_STP_HOME);
        Serial.print("homestat: ");Serial.println(homestat);
    }
